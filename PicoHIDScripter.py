...
class PicoHIDConverter:
    def convert_to_pico_script(pico_mnemonic):
        if pico_mnemonic.startswith("TIME"):
            return "import time"
        elif pico_mnemonic.startswith("BD"):
            return "import board"
            ...
            led_pin = int(pico_mnemonic.split(" ")[1])
            led_code = f"led = digitalio.DigitalInOut(board.GP{led_pin})\nled.direction = digitalio.Direction.OUTPUT"
            return led_code
        elif pico_mnemonic.startswith("LV"):
            led_pin_value = int(pico_mnemonic.split(" ")[1])
            return f"led.value = {led_pin_value}"
            ...
            return output_string
        elif pico_mnemonic.startswith("AR"):
            return "kbd.release_all()"
        elif pico_mnemonic.startswith("WHILE"):
            condition = int(pico_mnemonic.split(" ")[1])
