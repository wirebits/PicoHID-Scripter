...
class PicoHIDMain:
    ...
    def create_widgets(self):
        ...
        upload_button = ttk.Button(buttons_frame, text="Upload", command=self.upload_code)
        upload_button.pack(side="left", padx=5, pady=5)

        refresh_button = ttk.Button(buttons_frame, text="Refresh", command=self.display_com_port)
        refresh_button.pack(side="left", padx=5, pady=5)

        self.com_port_button = ttk.Label(buttons_frame, text="")
        self.com_port_button.pack(side="left", padx=5, pady=5)

        self.upload_status_label = ttk.Label(buttons_frame, text="")
        self.upload_status_label.pack(side="left", padx=5, pady=5)

        exit_button = ttk.Button(buttons_frame, text="Exit", command=self.exit_window)
        exit_button.pack(side="right", padx=5, pady=5)

    def clear_placeholder(self, event):
        if event.widget == self.mnemonic_frame and self.mnemonic_frame.get(1.0, tk.END).strip() == "Enter your mnemonic":
            self.mnemonic_frame.delete(1.0, tk.END)
        if event.widget == self.pico_frame and self.pico_frame.get(1.0, tk.END).strip() == "Your pico script":
    ...
    def display_com_port(self):
        ports = list(comports())
        if ports:
            self.com_port_button.config(text=f"{ports[0].device}")
        else:
            self.com_port_button.config(text="Not Connected")

    def upload_code(self):
        code = self.pico_frame.get(1.0, tk.END).strip()
        if not code:
            self.upload_status_label.config(text="No code to upload")
            return
        ...
        try:
            ...
            self.upload_status_label.config(text="Uploaded")
        except Exception as e:
            self.upload_status_label.config(text=f"Failed to upload: {e}")
